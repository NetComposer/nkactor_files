...
%% @doc NkACTOR Application Module
-module(nkactor_files_app).
...
start(_Type, _Args) ->
    ...
    case nklib_config:load_env(?APP, Syntax) of
        {ok, _} ->
            {ok, Pid} = nkactor_files_sup:start_link(),
            {ok, Vsn} = application:get_key(nkactor, vsn),
            Modules = [
                nkactor_files_filesystem_provider_actor,
                nkactor_files_s3_provider_actor,
                nkactor_files_file_actor
            ],
            nkactor_util:register_modules(?GROUP_FILES, Modules),
